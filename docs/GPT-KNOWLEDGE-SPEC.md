...
- 每個 `-` 節點 **≤ ~14 個全形字／約 22 ASCII**；寧可兩行短詞，不要長句
- 不要用 `痛點：`／`行動：` 這類前綴（會擠爆節點）；細節放 `notes/*.md`
- **不要**只放 mermaid fence；UI 會剝掉 ` ``` ` 再解析 `#` / `##` / `-`
- 主分支 `##` 標題用中文短名，並與 `NOTE_ALIASES` / `meta.json` label 對得上
...
| 點心智圖主分支（有「筆記」徽章） | 開啟對應 `note` |
| Claim / Actions | 色塊強調；標題顯示為「主張／下一步」 |

### 6.1 心智圖引擎（2026-09 更新）

- **離線內建 SVG 水平樹**（不依賴 CDN markmap；外網腳本會卡住載入）
- 解析 `mindmap.md` 的 `#` / `##` / `-`；用 Canvas `measureText` 換行，節點高度跟行數走
- **「筆記」徽章**畫在 SVG 上（不佔 label 高度），避免主分支標題被裁切
- **每主分支獨立配色**（填色／描邊／連線／接點）
- 預設 **依面板寬度縮放**並捲到根節點；工具列可切換「實際大小／符合視窗」
- CSS：`[hidden]` / `.is-hidden` 必須 `display:none !important`（含 `.map-status`）

若要換繪圖引擎，必須保持：**離線可畫**、**點主分支開筆記**、**短節點文字不跑框**。
